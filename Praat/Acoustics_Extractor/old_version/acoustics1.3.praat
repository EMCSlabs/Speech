...
#                             'number_of_phonemes' is not needed anymore
...
form Acoustics extractor
	...
	comment Analyze what phoneme sets (separated by comma with no space):
		word Phonemes aa,ee,ii,oo,uu
		boolean all_phonemes 
...
printline Num of Chunks: 'chunks'
printline ---------------------------------------------------------------------------------------------
...
if no_TextGrid = 1
	...
	titleline$ = "Filename,Pre,Phoneme,Post,"
	fileappend "'resultfile$'" 'titleline$'
endif
...
#---------------------- (7) standard deviation, skewness, kurtosis, center of gravity -----------------------------
	...
	date$ = date$()
	printline FileNum:'file_counting' 'date$' 'directory$''name$' ... done
...
for ifile from 1 to num_files
	...
	if fileReadable (tgname$)
		...
		# Check labels with predefined phoneme sets

		for iLabel from 1 to num_intervals
			# Extract phoneme one by one
			select 'textgrid'
			iPhoneme = 1
			ph_set$ = phonemes$
			p_label$ = Get label of interval... 'which_tier' 'iLabel'

#			while iPhoneme <= number_of_phonemes
#				commaLoc = index_regex(ph_set$,",")
#				ph_set_length = length(ph_set$)
#				if commaLoc = 0
#					one_phoneme$ = ph_set$
#				else
#					one_phoneme$ = left$(ph_set$,commaLoc-1)
#				endif
#				one_phoneme_length = length(one_phoneme$)
...
#------------------------------------------------------------------------------------------------------------------------
#------------(3) For specified phonemes ----------------------------------------------------------------------------
#------------------------------------------------------------------------------------------------------------------------
			elsif all_phonemes = 0
#------------------------------------------------------------------------------------------------------------------------
...
for ifile from 1 to num_files
	...
	if fileReadable (tgname$)
		...
		for iLabel from 1 to num_intervals
			...
			# Check phonemes
			if length(ph_set$) = 0
				exitScript: "Phonemes are not specified. Please type in phonemes first."
			endif

			# Count number_of_phonemes
			cnt = 1
			tmp$ = ph_set$
			val = index_regex(tmp$,",")
			long = length(tmp$)
			...
			while iPhoneme <= number_of_phonemes
				...
				##### Check if one_phoneme matches with the label #####
				if one_phoneme$ = p_label$
					select 'textgrid'
					p_b = Get starting point... which_tier iLabel
					p_e = Get end point... which_tier iLabel
					p_d = p_e - p_b	
					...
					# Write result file
					resultline$ = "'wavname$','pre$','one_phoneme$','post$',"
					fileappend "'resultfile$'" 'resultline$'
...
#---------------------- (7) standard deviation, skewness, kurtosis, center of gravity -----------------------------
				...
				# Update phoneme set for next loop
